# Create S-milinst_atop Using milinst_ATOP portfolios
library(tidyr)
library(dplyr)
library(foreign)
library(data.table)
...
### Let's look at the milinst_atop
foo<-portfolio_milinst_atop$`1816`
...
test<-foreach(i=1:length(milinst_atop), .combine=rbind.data.frame) %do% {
...
# save
save(S_milinst_atop, file="S_milinst_atop.Rdata")
...
# compare
check<-as.tbl(left_join(cleanDat, S_milinst_atop, by=c("ccode1", "ccode2", "year")))

# correlation
corr<-cor(check$s_un_glo, check$S_milinst_atop, use="pairwise")

# plot
g<- check %>%
        dplyr::select(ccode1, ccode2, year, s_un_glo, S_milinst_atop)

p<-ggplot(g, aes(x=S_milinst_atop, y=s_un_glo))+
